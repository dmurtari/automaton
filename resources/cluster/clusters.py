...
class Cluster(object):
    ...
    def terminate_all(self):
        ...
        if self.reservations:
            ...
            for cloud in self.clouds:
                reservations = cloud.conn.servers.list()
                print str(reservations)
        for instance in reservations:
            server = cloud.conn.servers.find(id=instance.id)
            server.delete()
...
class NimbusCluster(object):
    """NimbusCluster class represents resources used for a set of benchmarks running
    on a Nimbus cloud.
